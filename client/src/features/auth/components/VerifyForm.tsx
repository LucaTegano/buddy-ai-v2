...
const VerifyForm = () => {
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Type-safe extraction of the specific message from the backend response body
      let backendMessage = '';
      ...
      } else if (backendMessage.includes('Invalid') || backendMessage.includes('code')) {
        const errorMsg = t('verification.codeNotFound', 'Codice di verifica non valido. Controlla il codice e riprova.');
        setError(errorMsg);
        toast.error(errorMsg);
      } else {
        const errorMsg = backendMessage || t('verification.error', 'Si è verificato un errore imprevisto.');
        setError(errorMsg);
        toast.error(errorMsg);
      }
    }
