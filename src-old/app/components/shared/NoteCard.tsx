...
import { DocumentTextIcon, UsersIcon } from '@/app/components/icons';
...
const NoteCard: React.FC<NoteCardProps> = ({ note }) => {
    ...
    const { setActiveView, setActiveNoteId } = useAppStore();

    // No longer need handleClick as we're using Link navigation
    ...
    return (
        ...
        >
            ...
            <div className="text-xs text-text-secondary space-y-2">
                <p>{t('noteCard.lastEdited')} <span className="font-medium text-text-primary">{note.lastEdited}</span></p>
